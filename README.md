...
除 `TG-Auto` 外，候选池继续使用 fallback，而不是把全部节点放入
url-test。fallback 只在当前节点不可用时才进入下一个候选，避免视频业务因
毫秒级波动频繁换线。`TG-Auto` 仅对已经筛选出的最多 5 个节点使用
url-test：每 120 秒持续探测，候选延迟优势不足 150 ms 时保持当前节点，
连续失败 2 次或出现持续、明显的延迟劣化时才切换。
