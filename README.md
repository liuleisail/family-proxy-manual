...
根路径现在使用独立的 Vue 3 + TypeScript + Tailwind CSS 控制台，采用深浅色主题、响应式侧栏/移动端导航、状态卡片和 Lucide 线性图标。总览、家庭成员、流量观察、首次配置和系统维护五个视图直接读取现有 `/api/*` 接口；不具备后端历史数据或策略写入接口的内容会明确显示为当前快照或待接入，不生成假数据。
