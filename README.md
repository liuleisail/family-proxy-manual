...
- 升级前后检查管理服务的实际配置读取能力；只在宿主机正常、管理服务挂载明确失效时，备份并恢复对应管理服务。
- 每分钟自动检查，正在执行的配置事务和 DNS 更新会推迟恢复；连续恢复尝试至少间隔五分钟，不重启代理/DNS 核心或修改路由。
- Mihomo/MosDNS 核心健康但管理读取失败时单独报告，不把管理故障误当核心失败回滚。
- 规则和过滤页面结束无限加载并显示错误；部署同时更新 DNS 页面，安装后验证真实规则接口。

### `v0.11.16` 发布摘要

- 单设备清理按完整标签匹配；设备加入/撤出串行执行并采用跨进程锁，先准备 Z4Pro 转发再接通 RouterOS，回滚失败不再静默。
- 健康检查覆盖 TCP/UDP TPROXY 监听、策略路由、nft 规则和名单一致性；设备页面区分转发就绪、近期旁路流量和降级，普通直连连接不再算已生效。
- MosDNS 在备份、拉取和预检阶段失败时保持当前容器；只有进入实际切换后才执行旧镜像回滚。
- Docker 恢复只操作明确白名单内的容器，排除清单优先，支持维护暂停；默认白名单为空，不接管无关 NAS 服务。
- 修复 APNs 旧地址表清理的 RouterOS 菜单路径，以及部署同步目录结构、健康字段和版本指纹校验；保留已有 HomeKit 路由与 Mihomo 镜像挂载保护。

### `v0.11.15` 发布摘要

- 新版 Vue 机场候选池页面新增按业务池锁定机场范围；选择范围只影响下一轮测速，不改变当前生效出口。
- 全量稳定性测速完成后，新版页面会显示待生效的稳定候选；点击“复测并生效”后才写入实际候选池，未选择的池子保持不变。
- 修复页面重新加载后丢失待生效候选，以及已应用候选仍被旧建议覆盖的问题。

### `v0.11.14` 发布摘要

- 候选池支持先按业务池锁定机场范围，再执行全量稳定性测速；未选择的池子保持现有节点不变，YouTube 大流量池不会被这次 AI/TG 机场切换误改。
- “复测并生效”会重新测试候选节点，只有复测结果通过后才写入实际策略组，降低机场切换造成的瞬时中断风险。
- 部署脚本补充候选池运行时安装、Docker 恢复定时器以及 RouterOS 健康模板的边界校验，并保留发布前备份和 gated health 验证。
...
- 将新界面的 WireGuard“异地回家”设置迁移到旧版“维护”页面，支持公网端点、UDP 端口、家庭 DNS、客户端列表、撤销、二维码和 `.conf` 下载。
- 复用现有远程 WireGuard API，状态接口不返回私钥；生成客户端前仍需手动确认，部署不会修改已有 WireGuard 客户端。
- 修复 Z4Pro 源码同步和部署脚本的二维码资源校验与安装路径，补充维护页和部署链路回归测试。
- 已完成 Z4Pro 备份式部署、服务健康、WireGuard 状态和维护页二维码资源验证。

### `v0.11.9` 发布摘要

- 修复全新安装时 UI 服务因缺少审计日志文件触发 systemd `226/NAMESPACE` 的问题。
- 修复 Mihomo 基础配置引用未定义 `Apple`、`Telegram` 策略组导致容器反复重启的问题。
- 首装预创建 Mihomo `cache.db` 正规文件，避免 Docker 将其绑定为目录。
- 补充非 RB5009 路由器、独立旁路模式和设备 IP/代理端口的配置说明。

### `v0.11.6-z4pro-stable` Z4Pro 稳定版发布摘要

- 稳定基线：`v0.11.6`。
- 部署目标：当前 Z4Pro 线上环境。
- 状态：Stable，已完成 NAS/Z4Pro 部署及线上页面、运行状态校验。
- 已验证新旧管理界面切换入口和前端构建测试。
- 该版本只用于复现当前 Z4Pro 稳定环境，不包含 `v0.11.8`、`v0.11.9`、`v0.11.10` 的新增功能；需要最新功能请使用 `v0.11.10`。

### `v0.11.8` 发布摘要

- 新增维护页“异地回家”WireGuard 卡片，支持 RouterOS 7.21+ 自动创建独立接口、Peer、家庭 LAN 放行和 WAN 出口 NAT。
- 支持填写公网域名/IP，生成官方 WireGuard 客户端配置、二维码和 `.conf` 下载，并支持撤销客户端。
- 远程客户端使用 IPv4 全隧道 `AllowedIPs = 0.0.0.0/0`，可经家庭出口访问互联网和家庭 LAN；独立旁路模式不会写入 RouterOS。
- 增加 RouterOS API 回滚保护、WAN 检查、地址池冲突检查、密钥不回传状态接口，以及部署/服务器验证覆盖。
...
| 维护 | 检查 Mihomo、MosDNS、RouterOS 通道与 Z4Pro ZOS 更新状态；在 RouterOS 模式下生成异地回家 WireGuard 客户端 | 系统更新只检测和推送，绝不自动升级或重启设备；独立旁路模式不写入 RouterOS |
...
维护页的“异地回家”卡片只在 RouterOS 集成模式显示。它要求 RouterOS 7.21 或更高版本，并在第一次生成客户端时创建独立的 `family-remote-wg` 接口、地址池、家庭 LAN 访问规则和 WAN 出口 NAT；已有的 `back-to-home-vpn`、`wg-home` 以及其它非本系统 WireGuard 接口不会被修改。生成失败会清理本次新建的对象，不留下半套规则。

安装者需要在卡片中填写客户端名称、可从互联网访问的域名或公网 IP，以及映射到 RouterOS 的 UDP 端口。地址只做域名/公网地址格式校验；如果 RouterOS 位于上级 NAT 后面，还必须在上级设备转发该 UDP 端口。生成后页面一次性展示官方 WireGuard 配置和二维码，可直接用 iOS/Android 官方客户端扫描，也可下载 `.conf` 文件；状态接口不返回任何密钥。撤销客户端只删除该客户端，不影响其它客户端或现有隧道。

生成的客户端使用 `AllowedIPs = 0.0.0.0/0`，IPv4 流量全部经家庭出口，并允许访问家庭 LAN 网段，因此在外部网络可以像处于家庭内网一样访问 IPv4 家庭设备。默认 DNS 优先使用旁路主机上的 MosDNS，未配置时回退到 RouterOS DNS；需要 IPv6 全隧道时还需单独配置 IPv6 地址池、路由和防火墙，当前卡片不会伪造 IPv6 可用状态。
...
维护页的“系统更新与通知”每天约 09:15 检查一次（随机延后最多 10 分钟），并可手动检查。RouterOS 读取已设置的官方更新通道；Z4Pro 读取极空间 ZOS 自带升级服务的最近一次官方结果；Mihomo 与 MosDNS-T 仅执行镜像元数据比对。MosDNS-T 是第三方整合项目，版本信息读取其自身 Tags，不与官方 MosDNS 5.x 版本比较；实际升级仍以 `jasonxtt/mosdns-t:latest` 的镜像 digest 变化为准。Mihomo 只检测并展示正式发布版本，Alpha、Beta、RC、Nightly 等预发布版会被忽略。其余组件发现版本变化时，会使用已启用的 Telegram 告警仅推送一次；不执行 RouterOS、ZOS、Docker 或 `apt` 升级，也不会重启任何服务。系统更新请仍在 RouterOS 或极空间官方界面确认后手动完成。
...
2. 打开“机场与候选池 - 订阅来源”，粘贴到对应来源并点击“直连导入/替换”。首次导入会按节点名称自动生成启动候选池；已有规则、策略和候选池只做有效节点对账，不会被重置。
3. 等待页面报告导入和配置验证完成。导入失败时，现有可用配置应保持不变。
4. 导入完成后即可直接使用。之后可在“候选池”执行全量稳定性测速和“复测并生效”，用于把启动候选替换为经过三次连续测试的稳定顺序。管理系统不保存订阅链接。
...
完整订阅中的几十个节点只用于人工筛选；运行时只使用候选池中的显式节点。首次导入会先建立可运行的启动候选池：`视频` 是跨地域混合池，先接收可用节点；其它地区池按节点名称进入对应池；没有地区标签时，前几个节点进入通用 `Proxy` 池，相关业务先通过通用出口运行。每个稳定业务池建议保留 **1 至 5 个** 节点，稳定运行时维持 4 至 5 个：

- `视频` 不按机场地区或来源固定前置；可分别选择机场来源和地点范围，复测后以连续成功、中位延迟和抖动综合分排序。
- 所有备用机场合计补充 1 至 2 个节点。旧配置中的 `HK-视频` 是视频池的内部兼容名称。
- 排序即故障切换顺序；其它业务池仍按各自候选范围筛选。
- 没有某个备用机场时不留空位，不要为了凑数放低质量节点。

推荐业务池：`视频`、`JP-AI`、`SG-AI`、`US-AI`、`其他-AI`、`TG`、`Proxy`。`其他-AI` 承接台湾、韩国和其他非港、非日/新/美节点。AI、YouTube、Telegram、Google、Others 等业务策略只引用这些候选池，不能直接引用整份订阅。

### 测速建议与生效

1. 导入新来源后，在“候选池”点击“全量稳定性测速”。该操作仅在人工点击时运行，对每个节点连续测试三次。
2. 仅连续三次成功的节点可进入建议。系统按中位延迟和抖动排序，并自动填入每池 4 至 5 个节点；视频混合池不按机场来源固定前置。
3. `JP-AI`、`SG-AI`、`US-AI` 按地域筛选；`其他-AI` 收录台湾、韩国及其他非港节点，且不与前三池重复。所有 AI 候选池均排除香港节点。`视频` 支持分别锁定机场来源和地点（全部、香港、日本、新加坡、台湾、韩国、美国或其它），使用 YouTube 业务探针按连续成功、中位延迟和抖动排序；`TG`、`Proxy` 保持各自地域范围。
...
| YouTube | 视频（内部组 `HK-视频`） | YouTube 204 探测 | 300 秒 | 当前节点不可用 |
...
“数据管理 - 内容过滤”把国内 Lite 广告和成人内容规则分开维护，支持关闭、只记录不拦截的观察模式，以及返回 NXDOMAIN 的拦截模式。规则由服务器直连原始项目下载，本地剔除放行名单并校验后才交给 MosDNS；切换到拦截模式会执行固定国内外业务探针，失败自动恢复。首次部署建议先保持观察模式，确认购物、短视频、通信和 Apple 服务正常后再启用拦截。
