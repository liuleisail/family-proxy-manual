...
| 页面 | 日常用途 | 不做什么 |
| --- | --- | --- |
| 设备 | 查看旁路健康与 Z4Pro CPU、内存、温度、Docker 盘和容器状态；加入或撤出单台设备、改名、保留常用设备、查看流量计数 | 不在这里修改机场节点 |
...
首次操作前，确认管理页显示：Z4Pro 资源、路由器管理连接、DNS、Mihomo、当前策略和自动回退均为正常。Z4Pro 卡片每 10 秒更新，设备与流量列表每 30 秒更新。Docker 的“运行数 / 总数”不会启动已停止容器；温度工具不可用时只显示“不可用”，其它状态仍可读取。任意关键项异常时，先修复健康状态，不要继续接管新设备。
...
- 接管设备的中国 IPv4 流量在旁路主机内核中直接转发并 SNAT，不再穿过 Mihomo；国外 TCP/UDP 才进入 TPROXY。这样可避免京东、拼多多等多连接或视频业务受到透明代理开销影响。
- 中国 IPv4 集合每周从 APNIC 官方数据源通过本地直连更新，更新前校验条目数量，更新后只重载旁路转发规则，不重启 Docker 容器。
