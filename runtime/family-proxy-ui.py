...
def mihomo_component_update_status(check=False):
    ...
    # The deployed image tracks the Alpha tag. Keep it visible for a deliberate
    # manual maintenance action, but never treat prerelease builds as alerts.
    prerelease = any(marker in f"{current_version} {latest_version}".lower()
...
# Keep the device table readable.  The first-level action is intentionally only
# rename; operational actions are grouped in a compact per-device menu instead
# of competing with live state for horizontal space.
PAGE = PAGE.replace(
    "function deviceActions(d){let rename=`<button class=\"secondary action-rename\" onclick=\"openRename('${d.mac}')\">改名</button>`,homekit=d.homekit_direct?`<button class=\"secondary danger action-homekit\" onclick=\"setHomeKitDirect('${d.mac}',false)\">关闭 HomeKit</button>`:(filter==='online'||d.managed||d.favorite?`<button class=\"secondary action-homekit\" onclick=\"setHomeKitDirect('${d.mac}',true)\">启用 HomeKit</button>`:'');if(d.managed)return rename+homekit+`<button class=\"secondary action-link\" onclick=\"openEgress('${d.ip}')\">链路</button>`+`<button class=\"secondary action-diagnose\" onclick=\"openCapture('${d.ip}')\">诊断</button>`+(d.fixed?'<span class=\"action-fixed\">固定</span>':`<button class=\"secondary danger action-remove\" onclick=\"removeDevice('${d.ip}')\">恢复直连</button>`);let join=`<button class=\"secondary action-join\" onclick=\"choose('${d.ip}')\">加入旁路</button>`;if(filter==='favorites')return rename+homekit+join+`<button class=\"secondary danger action-remove\" onclick=\"setFavorite('${d.mac}',false)\">移出常用</button>`;let keep=d.favorite?`<button class=\"secondary danger action-remove\" onclick=\"setFavorite('${d.mac}',false)\">取消保留</button>`:`<button class=\"secondary action-join\" onclick=\"setFavorite('${d.mac}',true)\">保留</button>`;return rename+homekit+join+keep}",
    "function deviceMenu(label,content){return `<details class=\"device-menu\"><summary>${label}<span>⌄</span></summary><div class=\"device-menu-popover\">${content}</div></details>`}function deviceActions(d){let rename=`<button class=\"secondary action-rename\" onclick=\"openRename('${d.mac}')\">改名</button>`,homekit=d.homekit_direct?`<button class=\"secondary danger\" onclick=\"setHomeKitDirect('${d.mac}',false)\">关闭 HomeKit 本地直连</button>`:`<button class=\"secondary\" onclick=\"setHomeKitDirect('${d.mac}',true)\">启用 HomeKit 本地直连</button>`;if(d.managed){let more=homekit+`<button class=\"secondary\" onclick=\"openEgress('${d.ip}')\">查看出站链路</button><button class=\"secondary\" onclick=\"openCapture('${d.ip}')\">网络诊断</button>`+(d.fixed?`<span class=\"menu-note\">该设备的 DHCP 地址已固定</span>`:`<button class=\"secondary danger\" onclick=\"removeDevice('${d.ip}')\">恢复直连</button>`);return rename+deviceMenu('管理',more)}let join=`<button class=\"secondary\" onclick=\"choose('${d.ip}')\">加入旁路</button>`,keep=d.favorite?`<button class=\"secondary danger\" onclick=\"setFavorite('${d.mac}',false)\">移出常用设备</button>`:`<button class=\"secondary\" onclick=\"setFavorite('${d.mac}',true)\">加入常用设备</button>`;return rename+deviceMenu('管理',homekit+join+keep)}",
    1,
)
PAGE = PAGE.replace(
    '</style></head>',
    '''.device-header,.device-row{grid-template-columns:minmax(275px,1.42fr) minmax(180px,.86fr) 78px minmax(175px,.86fr) 132px}.device-header span:last-child{text-align:right}.device-action{display:flex;align-items:center;justify-content:flex-end;gap:7px;position:relative;padding:0 16px!important}.device-action .secondary{min-width:0;padding:7px 9px;white-space:nowrap}.device-action .action-rename{background:transparent;color:#0a84ff}.device-action .action-rename:hover{background:rgba(10,132,255,.12)}.device-menu{position:relative}.device-menu>summary{display:flex;align-items:center;gap:5px;min-width:58px;height:32px;padding:0 9px;border-radius:6px;background:#2c2c2e;color:#f5f5f7;cursor:pointer;font-size:12px;font-weight:600;list-style:none;white-space:nowrap}.device-menu>summary::-webkit-details-marker{display:none}.device-menu>summary span{color:#8e8e93;font-size:13px}.device-menu[open]>summary{background:#3a3a3c}.device-menu-popover{position:absolute;z-index:8;top:38px;right:0;display:none;width:205px;padding:6px;border:1px solid #48484a;border-radius:8px;background:#242426;box-shadow:0 12px 32px rgba(0,0,0,.42)}.device-menu[open] .device-menu-popover{display:grid;gap:2px}.device-menu-popover .secondary{width:100%;min-height:34px;background:transparent;text-align:left;color:#f5f5f7}.device-menu-popover .secondary:hover{background:#363638}.device-menu-popover .danger{color:#ff6961}.menu-note{padding:8px 9px;color:#8e8e93;font-size:11px;line-height:1.4}@media(max-width:1180px){.device-header,.device-row{grid-template-columns:minmax(225px,1.32fr) minmax(150px,.8fr) 66px minmax(145px,.78fr) 126px}.device-header span,.device-row>div{padding-left:14px;padding-right:14px}.device-action{padding-left:6px!important;padding-right:10px!important}}@media(max-width:860px){.device-header{display:none}.device-action{padding:0!important;justify-content:flex-end}.device-menu-popover{right:0}.device-menu>summary{min-width:54px}}@media(max-width:460px){.device-action{gap:2px}.device-action .action-rename{padding-left:5px;padding-right:5px}.device-menu-popover{width:194px}}</style></head>''',
    1,
)
