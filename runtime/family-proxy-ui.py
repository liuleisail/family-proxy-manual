...
COMPONENT_RELEASE_SOURCES = {
    ...
    "routeros": {
        ...
        "url": "https://mikrotik.com/download/changelogs?channelFilter=stable",
    },
    "z4pro": {
        "label": "极空间官方升级服务",
...
PAGE = PAGE.replace(
    "</style></head>",
    ".dash-warning{position:relative;display:inline-grid;place-items:center;width:15px;height:15px;margin-left:6px;border-radius:50%;background:#5a5a5e;color:#f5f5f7;font-size:11px;font-weight:800;vertical-align:1px;cursor:help;overflow:visible!important;white-space:normal!important}.dash-warning:after{position:absolute;z-index:9999;bottom:calc(100% + 8px);left:50%;width:max-content;max-width:min(300px,70vw);padding:7px 9px;border:1px solid #48484a;border-radius:6px;background:#3a3a3c;color:#fff;box-shadow:0 8px 20px rgba(0,0,0,.5);content:attr(data-tooltip);font-size:12px;font-weight:500;line-height:1.45;opacity:0;pointer-events:none;transform:translate(-50%,4px);transition:opacity .08s ease,transform .08s ease;white-space:normal}.dash-warning:hover:after,.dash-warning:focus-visible:after{opacity:1;transform:translate(-50%,0)}.dash-dns.bad .dash-warning{background:#5a5a5e;color:#f5f5f7}</style></head>",
    1,
)
PAGE = PAGE.replace(
    '<small>自动回退</small>',
    '<small>自动回退<span class="dash-warning" tabindex="0" data-tooltip="保护性自动化：RouterOS Netwatch 连续探测异常时，会临时切换出口并清理旧连接；探针恢复后自动启用。感叹号表示该机制会影响旁路状态，不代表当前故障。" aria-label="保护性自动化：RouterOS Netwatch 连续探测异常时，会临时切换出口并清理旧连接；探针恢复后自动启用。感叹号表示该机制会影响旁路状态，不代表当前故障。">!</span></small>',
    1,
)
PAGE = PAGE.replace(
...
_REMOTE_WG_CARD = r'''<section class="card remote-wg-card"><div class="card-head"><div><h2>异地回家</h2><p>生成官方 WireGuard 客户端配置；连接后 IPv4 流量经家庭出口，并可访问家庭 LAN 设备。</p></div><span id="remote-wg-badge" class="badge">读取中</span></div><div id="remote-wg-unavailable" class="remote-wg-unavailable" hidden></div><div id="remote-wg-layout" class="remote-wg-layout"><form id="remote-wg-form" class="remote-wg-form"><div class="remote-wg-fields"><label>客户端名称<input id="remote-wg-name" maxlength="40" autocomplete="off" placeholder="例如 iPhone 16 Pro Max"></label><label>外部域名或公网 IP<input id="remote-wg-endpoint" maxlength="253" autocomplete="off" placeholder="例如 vpn.example.com"></label><label>WireGuard UDP 端口<input id="remote-wg-port" inputmode="numeric" type="number" min="1" max="65535" value="51820"></label><label>家庭 DNS<input id="remote-wg-dns" autocomplete="off" placeholder="默认使用旁路主机 MosDNS"></label></div><div id="remote-wg-detail" class="detail-inline">首次生成时会创建独立的 WireGuard 接口和家庭出口 NAT。</div><button id="remote-wg-generate" class="primary" type="submit">+ 生成客户端二维码</button></form><div class="remote-wg-client-list"><div class="remote-wg-list-head"><span>已生成客户端</span><strong id="remote-wg-count">0</strong></div><div id="remote-wg-list" class="remote-wg-list"><div class="empty-inline">正在读取远程客户端</div></div></div></div><div id="remote-wg-dialog" class="remote-wg-dialog" hidden><div class="remote-wg-dialog-card"><button id="remote-wg-close" class="dialog-close" type="button" title="关闭二维码" aria-label="关闭二维码">x</button><span class="eyebrow">WIREGUARD CLIENT</span><h2 id="remote-wg-dialog-title">WireGuard 客户端</h2><p id="remote-wg-dialog-detail">使用官方 WireGuard 客户端扫描二维码。二维码包含私钥，请勿转发。</p><div class="remote-wg-qr"><img id="remote-wg-qr-image" alt="WireGuard 客户端二维码"></div><div class="remote-wg-dialog-actions"><button id="remote-wg-download" type="button">下载 .conf</button><button id="remote-wg-done" class="primary" type="button">完成</button></div></div></div></section>'''

_REMOTE_WG_SCRIPT = r'''function remoteWgEsc(value){return String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]))}function remoteWgBytes(value){let number=Number(value||0);if(!Number.isFinite(number)||number<0)return '--';let units=['B','KB','MB','GB','TB'],index=0;while(number>=1024&&index<units.length-1){number/=1024;index++}return `${number.toFixed(index?1:0)} ${units[index]}`}function remoteWgState(client){return client.active?'在线':client.last_handshake_seconds===null||client.last_handshake_seconds===undefined?'等待连接':'已离线'}function remoteWgTraffic(client){return `↓ ${remoteWgBytes(client.rx_bytes)} · ↑ ${remoteWgBytes(client.tx_bytes)}`}function remoteWgRender(data){let supported=data.supported!==false,badge=document.querySelector('#remote-wg-badge'),unavailable=document.querySelector('#remote-wg-unavailable'),layout=document.querySelector('#remote-wg-layout'),detail=document.querySelector('#remote-wg-detail'),port=document.querySelector('#remote-wg-port');badge.textContent=!supported?'不可用':data.interface?.running?'接口在线':data.interface?'已创建':'待创建';badge.className='badge '+(!supported?'bad':data.interface?.running?'':'warn');unavailable.hidden=supported;unavailable.textContent=supported?'':data.message||'当前模式不支持自动生成 RouterOS WireGuard 客户端配置。';layout.hidden=!supported;if(data.interface){port.value=data.interface.listen_port||port.value;detail.textContent=`服务端 ${data.interface.name} · ${data.interface.network} · UDP ${data.interface.listen_port}；${data.message||'客户端可访问家庭 LAN'}`}let clients=data.clients||[];document.querySelector('#remote-wg-count').textContent=clients.length;document.querySelector('#remote-wg-list').innerHTML=clients.length?clients.map(client=>`<div class="remote-wg-client"><div><strong>${remoteWgEsc(client.name||'未命名客户端')}</strong><small>${remoteWgEsc(client.address||'未分配地址')} · ${remoteWgTraffic(client)}</small></div><div class="remote-wg-client-actions"><span class="${client.active?'good':''}">${remoteWgState(client)}</span><button class="remote-wg-revoke" type="button" data-id="${remoteWgEsc(client.id||'')}" title="撤销客户端" aria-label="撤销客户端">撤销</button></div></div>`).join(''):'<div class="empty-inline">还没有生成远程客户端</div>';document.querySelectorAll('.remote-wg-revoke').forEach(button=>button.onclick=()=>remoteWgRevoke(button.dataset.id))}async function loadRemoteWg(){try{remoteWgRender(await api('/api/wireguard/remote-access'))}catch(e){let badge=document.querySelector('#remote-wg-badge');badge.textContent='读取失败';badge.className='badge bad';document.querySelector('#remote-wg-detail').textContent=e.message}}let remoteWgResult=null;async function remoteWgGenerate(event){event.preventDefault();let name=document.querySelector('#remote-wg-name').value.trim(),endpoint=document.querySelector('#remote-wg-endpoint').value.trim(),button=document.querySelector('#remote-wg-generate');if(!name||!endpoint){document.querySelector('#remote-wg-detail').textContent='请填写客户端名称和公网域名/IP';return}if(!confirm(`将在 RouterOS 创建「${name}」的 WireGuard 客户端，并允许其访问家庭 LAN。继续吗？`))return;button.disabled=true;button.textContent='正在生成…';try{let result=await api('/api/wireguard/remote-access/generate',{method:'POST',body:JSON.stringify({name,endpoint,port:Number(document.querySelector('#remote-wg-port').value),dns:document.querySelector('#remote-wg-dns').value.trim()})}),qr=await FamilyQRCode.toDataURL(result.config,{width:292,margin:2,errorCorrectionLevel:'M'});remoteWgResult={name:result.client.name,address:result.client.address,config:result.config};document.querySelector('#remote-wg-dialog-title').textContent=result.client.name;document.querySelector('#remote-wg-dialog-detail').textContent=`使用官方 WireGuard 客户端扫描二维码；客户端地址 ${result.client.address}。二维码包含私钥，请勿转发。`;document.querySelector('#remote-wg-qr-image').src=qr;document.querySelector('#remote-wg-dialog').hidden=false;document.querySelector('#remote-wg-detail').textContent=result.message||'客户端配置已生成';await loadRemoteWg()}catch(e){document.querySelector('#remote-wg-detail').textContent='操作失败：'+e.message}finally{button.disabled=false;button.textContent='+ 生成客户端二维码'}}async function remoteWgRevoke(id){if(!id||!confirm('撤销该客户端后，已导入的配置将立即失效。继续吗？'))return;try{let result=await api('/api/wireguard/remote-access/revoke',{method:'POST',body:JSON.stringify({id})});document.querySelector('#remote-wg-detail').textContent=result.message||'客户端已撤销';await loadRemoteWg()}catch(e){document.querySelector('#remote-wg-detail').textContent='操作失败：'+e.message}}function remoteWgClose(){document.querySelector('#remote-wg-dialog').hidden=true}function remoteWgDownload(){if(!remoteWgResult)return;let filename=(remoteWgResult.name.replace(/[^\w.-]+/g,'-')||'family-wireguard')+'.conf',url=URL.createObjectURL(new Blob([remoteWgResult.config],{type:'text/plain;charset=utf-8'})),anchor=document.createElement('a');anchor.href=url;anchor.download=filename;anchor.click();URL.revokeObjectURL(url)}document.querySelector('#remote-wg-form').addEventListener('submit',remoteWgGenerate);document.querySelector('#remote-wg-close').onclick=remoteWgClose;document.querySelector('#remote-wg-done').onclick=remoteWgClose;document.querySelector('#remote-wg-download').onclick=remoteWgDownload;document.querySelector('#remote-wg-dialog').addEventListener('click',event=>{if(event.target===event.currentTarget)remoteWgClose()});loadRemoteWg();setInterval(()=>{if(document.querySelector('#remote-wg-dialog').hidden)loadRemoteWg()},10000);'''
...
MIHOMO_MAINTENANCE_PAGE = MIHOMO_MAINTENANCE_PAGE.replace(
    ...
    '<p class="notice">', _REMOTE_WG_CARD + '<p class="notice">', 1,
).replace(
    '</style>',
    r'''.remote-wg-card{margin-top:14px}.remote-wg-layout{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(260px,.85fr);gap:22px;padding:16px}.remote-wg-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.remote-wg-fields label{display:block;color:#aeaeb2;font-size:12px}.remote-wg-fields input{width:100%;min-width:0;height:36px;margin-top:7px;padding:0 10px;border:1px solid #48484a;border-radius:7px;background:#2c2c2e;color:#f5f5f7;font:14px inherit}.remote-wg-fields input:focus{border-color:#0a84ff;outline:0;box-shadow:0 0 0 3px rgba(10,132,255,.18)}.detail-inline{margin:14px 0;color:#8e8e93;font-size:12px;line-height:1.55}.remote-wg-form>button{height:38px}.remote-wg-unavailable{margin:16px;padding:12px 13px;border-left:3px solid #ffd60a;background:rgba(255,214,10,.12);color:#d1d1d6;font-size:12px;line-height:1.55}.remote-wg-client-list{min-width:0;padding-left:20px;border-left:1px solid #38383a}.remote-wg-list-head{display:flex;align-items:center;justify-content:space-between;color:#8e8e93;font-size:12px}.remote-wg-list-head strong{color:#f5f5f7;font-size:16px}.remote-wg-list{display:grid;margin-top:7px}.remote-wg-client{display:flex;min-width:0;align-items:center;justify-content:space-between;gap:12px;padding:11px 0;border-top:1px solid #38383a}.remote-wg-client>div:first-child{min-width:0}.remote-wg-client strong,.remote-wg-client small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.remote-wg-client strong{color:#f5f5f7;font-size:13px}.remote-wg-client small{margin-top:4px;color:#8e8e93;font-size:11px}.remote-wg-client-actions{display:flex;flex:0 0 auto;align-items:center;gap:7px}.remote-wg-client-actions>span{color:#8e8e93;font-size:11px;white-space:nowrap}.remote-wg-client-actions>span.good{color:#30d158}.remote-wg-revoke{height:30px;padding:0 9px;border:1px solid #48484a;border-radius:6px;background:#2c2c2e;color:#ff6961;font-size:12px}.remote-wg-dialog{position:fixed;inset:0;z-index:20;display:grid;place-items:center;padding:20px;background:rgba(0,0,0,.72)}.remote-wg-dialog[hidden]{display:none}.remote-wg-dialog-card{position:relative;width:min(390px,100%);padding:20px;border:1px solid #48484a;border-radius:8px;background:#1c1c1e;box-shadow:0 22px 70px rgba(0,0,0,.65)}.remote-wg-dialog-card h2{margin:7px 0 0;font-size:18px}.remote-wg-dialog-card p{margin:8px 0;color:#8e8e93;font-size:12px;line-height:1.55}.dialog-close{position:absolute;top:14px;right:14px;width:30px;height:30px;padding:0;border:1px solid #48484a;border-radius:50%;background:#2c2c2e;color:#d1d1d6;font-size:18px;line-height:1}.eyebrow{display:block;color:#8e8e93;font-size:11px;font-weight:700;letter-spacing:1.6px}.remote-wg-qr{display:grid;width:fit-content;margin:20px auto 4px;padding:12px;border-radius:9px;background:#fff}.remote-wg-qr img{display:block;width:292px;height:292px}.remote-wg-dialog-actions{display:flex;justify-content:flex-end;gap:9px;margin-top:16px}.remote-wg-dialog-actions button{height:36px}@media(max-width:760px){.remote-wg-layout{grid-template-columns:1fr;gap:17px}.remote-wg-client-list{padding-top:17px;padding-left:0;border-top:1px solid #38383a;border-left:0}}@media(max-width:480px){.remote-wg-fields{grid-template-columns:1fr}.remote-wg-qr img{width:245px;height:245px}.remote-wg-dialog-actions{flex-direction:column}.remote-wg-dialog-actions button{width:100%}}'''+ '</style>',
    1,
).replace(
