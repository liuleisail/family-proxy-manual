...
PAGE = PAGE.replace(
    '<small>自动回退</small>',
    '<small>自动回退<span class="dash-warning" tabindex="0" data-tooltip="保护性自动化：RouterOS Netwatch 连续探测异常时，会临时切换出口并清理旧连接；探针恢复后自动启用。黄色叹号表示该机制会影响旁路状态，不代表当前故障。" aria-label="保护性自动化：RouterOS Netwatch 连续探测异常时，会临时切换出口并清理旧连接；探针恢复后自动启用。黄色叹号表示该机制会影响旁路状态，不代表当前故障。">!</span></small>',
    1,
)
PAGE = PAGE.replace(
...
PAGE = PAGE.replace(
    '</style></head>',
    '.dash-core-item small .dash-warning{display:inline-grid;flex:0 0 15px;width:15px;height:15px;margin:0 0 0 1px;overflow:visible!important;color:#2c2416;font-size:11px;line-height:15px;white-space:normal!important}</style></head>',
    1,
)
PAGE = PAGE.replace(
    '</style></head>',
