...
function mount(){{let host=document.querySelector('.topbar-inner')||document.querySelector('header')||document.body;let button=document.createElement('button');button.type='button';button.className='family-layout-settings';button.title='页面显示设置';button.setAttribute('aria-label','页面显示设置');button.textContent='⚙';button.onclick=open;host.append(button);let dialog=document.createElement('dialog');dialog.id='family-layout-dialog';dialog.className='family-layout-dialog';dialog.innerHTML='<div class="family-layout-head"><h2>页面显示</h2><p>拖动手柄或使用箭头排序；标记为固定显示的核心入口不能隐藏，其余区块可按需隐藏。</p></div><div id="family-layout-list" class="family-layout-list"></div><div class="family-layout-foot"><span id="family-layout-status" class="family-layout-status"></span><div class="family-layout-actions"><button type="button" id="family-layout-reset">恢复默认</button><button type="button" class="primary" id="family-layout-save">保存</button></div></div>';document.body.append(dialog);dialog.addEventListener('click',event=>{{if(event.target===dialog)close()}});document.querySelector('#family-layout-reset').onclick=()=>save(true);document.querySelector('#family-layout-save').onclick=()=>save(false);request('/api/page-layout').then(data=>{{let prefs=data[page]||{{}};hidden=new Set(prefs.hidden||[]);order=normalizeOrder(prefs.order);renderList();apply()}}).catch(error=>{{renderList();status('设置读取失败：'+error.message,true)}})}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',mount);else mount();
    ...
    enhancements = ('''<style>
.family-layout-moves{display:none!important}.family-layout-item{grid-template-columns:16px 16px minmax(0,1fr) 16px!important}.family-layout-handle{touch-action:none;cursor:grab}.family-layout-item.family-layout-touch-dragging{opacity:.46;cursor:grabbing}.family-layout-expand{display:grid;place-items:center;width:16px;height:16px;margin:0}.family-layout-expand input{cursor:pointer}.family-layout-expand-label{display:none}
</style><script>(()=>{
...
function updateDialogHint(){let hint=document.querySelector('.family-layout-head p'),text='拖住左侧手柄调整顺序；勾选可隐藏非核心区块；最右侧开关用于设置折叠区块的默认展开状态。';if(hint&&hint.textContent!==text)hint.textContent=text}
new MutationObserver(()=>{enhanceList();updateDialogHint()}).observe(document.documentElement,{childList:true,subtree:true});interceptActions();loadState();updateDialogHint();
    ...
    return html.replace("</body>", client + enhancements + "</body>")
