...
function applyAirportState(hidden){if(layoutPage!=='airport')return;let isHidden=(hidden||[]).includes('switch_history'),button=[...document.querySelectorAll('.tabs button')].find(node=>node.textContent.trim()==='切换状态'),panel=document.querySelector('#runtime');if(button)button.classList.toggle('family-layout-hidden',isHidden);if(panel)panel.classList.toggle('family-layout-hidden',isHidden);if(isHidden&&panel?.classList.contains('on'))document.querySelector('.tabs button')?.click();document.querySelector('#filter')?.remove()}
