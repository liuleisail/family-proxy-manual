...
def inject_page_layout(html, page):
    ...
    if not sections or "family-layout-settings" in html:
        return html
    settings_button = f'<button type="button" class="family-layout-settings" title="页面显示设置" aria-label="页面显示设置">{SETTINGS_ICON}</button>'
    if '<div class="topbar-inner">' in html or '<div class="header-inner">' in html:
        html = re.sub(
            r'(<div class="(?:topbar-inner|header-inner)">.*?)(</div>\s*</header>)',
...
function mount(){{if(document.querySelector('#family-layout-dialog'))return;let host=document.querySelector('.topbar-inner')||document.querySelector('.header-inner')||document.querySelector('header')||document.body;let button=document.querySelector('.family-layout-settings');if(!button){{button=document.createElement('button');button.type='button';button.className='family-layout-settings';button.title='页面显示设置';button.setAttribute('aria-label','页面显示设置');button.innerHTML={settings_icon};host.append(button)}}button.onclick=open;let dialog=document.createElement('dialog');dialog.id='family-layout-dialog';dialog.className='family-layout-dialog';dialog.innerHTML='<div class="family-layout-head"><h2>页面显示</h2><p>拖动手柄或使用箭头排序；标记为固定显示的核心入口不能隐藏，其余区块可按需隐藏。</p></div><div id="family-layout-list" class="family-layout-list"></div><div class="family-layout-foot"><span id="family-layout-status" class="family-layout-status"></span><div class="family-layout-actions"><button type="button" id="family-layout-reset">恢复默认</button><button type="button" class="primary" id="family-layout-save">保存</button></div></div>';document.body.append(dialog);dialog.addEventListener('click',event=>{{if(event.target===dialog)close()}});document.querySelector('#family-layout-reset').onclick=()=>save(true);document.querySelector('#family-layout-save').onclick=()=>save(false);request('/api/page-layout').then(data=>{{let prefs=data[page]||{{}};hidden=new Set(prefs.hidden||[]);order=normalizeOrder(prefs.order);renderList();apply()}}).catch(error=>{{renderList();status('设置读取失败：'+error.message,true)}})}}
