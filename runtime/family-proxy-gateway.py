...
function applyAirportState(hidden){if(layoutPage!=='airport')return;let isHidden=(hidden||[]).includes('switch_history'),button=[...document.querySelectorAll('.tabs button')].find(node=>node.textContent.trim()==='切换状态'),panel=document.querySelector('#runtime');if(!isHidden)return;if(panel?.classList.contains('on'))document.querySelector('.tabs button')?.click();button?.remove();panel?.remove();document.querySelector('#filter')?.remove()}
