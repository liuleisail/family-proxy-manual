...
class FirstRunSetupTests(unittest.TestCase):
    ...
    def test_mosdns_dashboard_keeps_upstreams_line_based_and_race_discoverable(self):
        ...
        self.assertIn('function openRacePopover(group)', DASHBOARD_SOURCE)
        self.assertIn('function closeRacePopover(group)', DASHBOARD_SOURCE)
        self.assertIn('data-race-close="${group}"', DASHBOARD_SOURCE)
        ...
        self.assertIn('class="route-upstream"><span>上游</span><span class="route-addresses">', DASHBOARD_SOURCE)
        self.assertNotIn("上游${index ? ` ${index + 1}` : ''}", DASHBOARD_SOURCE)
        self.assertNotIn("${label} ${index + 1}", DASHBOARD_SOURCE)
        self.assertNotIn("<span>上游</span><span title=", DASHBOARD_SOURCE)
