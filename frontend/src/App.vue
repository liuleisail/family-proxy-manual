...
const ruleSetPresetItems: JsonRecord[] = [
  ...
  { id: 'apple-apns-classical', name: 'Apple APNs', group: 'Apple APNs', url: 'https://raw.githubusercontent.com/mrbruce516/apns-fix/refs/heads/main/Apple_APNs.list', behavior: 'classical', format: 'text', policy: 'Proxy-Auto', priority: 'high', interval: 86400 },
...
async function loadAirportState() {
  try {
    ...
    const suggestion = merged.suggestions && typeof merged.suggestions === 'object' ? merged.suggestions : {}
    const pendingPools = suggestion.ready && !suggestion.applied_at && suggestion.pools && typeof suggestion.pools === 'object'
      ? suggestion.pools as Record<string, string[]>
      : null
    airportState.value = merged
    airportSourceSelections.value = { ...(merged.source_selections || {}) }
    airportPools.value = Object.fromEntries(Object.entries(pendingPools || merged.pools || {}).map(([key, values]) => [key, [...values]]))
...
function airportSourceLabelForSlot(slot: string) {
...
function mosdnsFailureText() {
  const failure = (mosdnsStatus.value.last_failure as { message?: string } | undefined)?.message
  return failure ? ` · 上次升级失败：${failure}` : ''
}
...
async function applyMosdns() {
  if (busy.value === 'mosdns-apply') return
  busy.value = 'mosdns-apply'
  try {
    if (!window.confirm('升级将短暂重启 MosDNS，失败时自动回退。继续吗？')) return
    await dnsMaintenanceApi('/update', { method: 'POST', body: '{}' })
    notify('MosDNS 升级已启动，正在等待验证结果…')
    const active = new Set(['busy', 'checking', 'updating', 'rolling_back'])
    const started = Date.now()
    while (Date.now() - started < 35 * 60 * 1000) {
      await new Promise((resolve) => setTimeout(resolve, 5000))
      await loadOpsFeature()
      if (!active.has(String(mosdnsStatus.value.phase || ''))) break
    }
    const phase = String(mosdnsStatus.value.phase || '')
    const lastFailure = (mosdnsStatus.value.last_failure as { message?: string } | undefined)?.message
    if (phase === 'updated' || phase === 'success') notify('MosDNS 升级完成并验证通过')
    else if (phase === 'rolled_back') notify(`MosDNS 升级失败已自动回退${lastFailure ? `：${lastFailure}` : ''}`)
    else if (active.has(phase)) notify('MosDNS 升级仍在进行，可稍后在维护页查看最终状态')
    else notify('MosDNS 升级流程已结束')
  } catch (error) {
...
async function applyMihomo() {
  if (busy.value === 'mihomo-apply') return
  busy.value = 'mihomo-apply'
  try {
  ...
  <div class="app-frame">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom">
        ...
        <div class="sidebar-version">Family Proxy <span :title="String(summary.build_id || '')">{{ summary.version || '0.11.17' }}</span></div>
      </div>
    </aside>
      ...
      <div class="page-wrap">
        ...
        <section v-if="activeView === 'overview'" class="view-panel">
          ...
          <div class="metric-grid">
            ...
            <article class="metric-card metric-card-action accent-green" role="button" tabindex="0" title="打开流量观察" @click="selectView('traffic')" @keyup.enter="selectView('traffic')"><div class="metric-icon"><Wifi :size="19" /></div><span class="metric-label">活动连接</span><strong>{{ totalConnections }}</strong><small>当前连接数</small><ArrowUpRight class="metric-arrow" :size="16" /></article>
