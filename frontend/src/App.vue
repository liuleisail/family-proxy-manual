  ...
  <div class="app-frame">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom">
        ...
        <div class="sidebar-version">Family Proxy <span :title="String(summary.build_id || '')">{{ summary.version || '0.11.10' }}</span></div>
      </div>
    </aside>
    ...
    <main class="main-content">
      ...
      <div class="page-wrap">
        ...
        <section v-if="activeView === 'overview'" class="view-panel">
          <div class="page-heading hero-heading"><div><span class="eyebrow">{{ standaloneMode ? 'STANDALONE ROUTING' : 'SELECTIVE ROUTING' }} · {{ ready ? 'LIVE' : 'CHECK' }}</span><h1>{{ standaloneMode ? '独立旁路，按需使用。' : '让家庭网络，' }}<em v-if="!standaloneMode">自然地工作。</em></h1><p>{{ standaloneMode ? '旁路主机不会读取或接管家庭设备；请手动设置代理，或在家庭网关配置策略路由。' : '一眼掌握旁路状态、设备接管和当前流量，让每个连接都走在正确的路径上。' }}</p></div><div class="heading-status" :class="{ warning: !ready }"><span class="status-dot" /><div><strong>{{ ready ? (standaloneMode ? '独立旁路已就绪' : '网络状态良好') : '网络需要检查' }}</strong><small>刚刚更新 · 自动刷新 30 秒</small></div></div></div>
        ...
        <section v-else-if="activeView === 'airport'" class="view-panel feature-view">
          ...
          <template v-if="airportTab === 'sources'">
            <section class="surface-card source-panel"><div class="card-heading"><div><span class="eyebrow">DIRECT IMPORT</span><h2>订阅来源</h2><p class="card-subtitle">{{ standaloneMode ? '由独立旁路主机直连拉取，不经过家庭网关代理、Fake-IP 或第三方转换；导入失败时保留当前可用配置。' : '由 Z4Pro 直连拉取，不经过代理、Fake-IP 或第三方转换；导入失败时保留当前可用配置。' }}</p></div><button class="secondary-button" @click="addAirportSource"><Plus :size="15" />新增备用机场</button></div><div class="source-columns" aria-hidden="true"><span>来源</span><span>HTTPS 原生 Clash / Mihomo 订阅地址</span><span>操作</span></div><div class="source-list"><article v-for="source in airportSources" :key="source.slot" class="source-row"><div class="source-name"><span class="source-index">{{ source.slot === 'primary' ? '01' : '02' }}</span><div><strong>{{ source.label }}</strong><span class="source-state" :class="{ empty: !source.imported }">{{ source.imported ? `已导入 ${source.nodes || 0} 个有效节点` : '尚未导入' }}</span><small class="source-updated">{{ source.imported ? `最后更新 · ${source.updated_at}` : '导入后可在候选池中选择节点' }}</small></div></div><input v-model="airportUrls[source.slot]" type="url" autocomplete="off" placeholder="HTTPS 原生 Clash/Mihomo 订阅链接" /><div class="source-actions"><small>{{ source.imported ? '当前生效来源' : '备用来源' }}</small><button class="primary-button" :disabled="busy === `airport-import-${source.slot}`" @click="importAirport(source)"><ArrowUpRight :size="15" />直连导入或替换</button><button class="secondary-button" @click="clearAirportSource(source)">清空</button><button v-if="source.slot !== 'primary'" class="icon-button small" title="删除机场来源" aria-label="删除机场来源" @click="removeAirportSource(source)"><X :size="15" /></button></div></article></div></section>
          </template>
