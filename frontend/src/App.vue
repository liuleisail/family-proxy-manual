...
type WireGuardPeer = { id?: string; name?: string; display_name?: string; default_label?: string; alias?: string; alias_key?: string; endpoint?: string; last_handshake_seconds?: number; state?: string; state_text?: string; allowed_address?: string; rx_bytes?: number; tx_bytes?: number }
type WireGuardInterface = { name?: string; label?: string; default_label?: string; alias?: string; alias_key?: string; kind?: string; peers?: WireGuardPeer[]; state?: string; state_text?: string; listen_port?: string; peer_total?: number; peer_active?: number; last_handshake_seconds?: number; rx_bytes?: number; tx_bytes?: number }
      ...
      <div class="page-wrap">
        ...
        <section v-if="activeView === 'overview'" class="view-panel">
          ...
          <div class="section-grid overview-grid">
            ...
            <section class="surface-card insight-card"><div class="card-heading"><div><span class="eyebrow">NETWORK OVERVIEW</span><h2>家庭网络概况</h2></div><MoreHorizontal :size="19" class="muted-icon" /></div><div class="network-visual"><div class="network-node router-node"><Router :size="24" /><span>RB5009</span><small>{{ summary.router === 'connected' ? '已连接' : '未连接' }}</small></div><div class="network-line"><span /><span /><span /></div><div class="network-node proxy-node"><div class="proxy-icon"><div class="node-pulse" /><ShieldCheck :size="24" /></div><span>旁路控制面</span><small>{{ checks.mihomo ? 'Mihomo 在线' : '控制接口不可用' }}</small></div></div><div class="insight-footer"><div><small>自动回退</small><strong>{{ summary.netwatch === 'up' ? '已启用' : '未就绪' }}</strong></div><div><small>远程互联</small><strong>{{ wireguardPeers }} 个 Peer</strong></div><div><small>CPU</small><strong>{{ number(system.cpu?.percent).toFixed(1) }}%</strong></div></div></section>
          </div>
