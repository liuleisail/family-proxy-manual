...
2. 在需要更换来源的业务池中选择机场并点击“锁定机场范围”；视频池还可选择地点（全部、香港、日本、新加坡、台湾、韩国、美国或其它）。这些操作只保存待测范围，不改变当前出口。
3. 执行“全量稳定性测速”；系统只对已锁定机场和地点范围内、符合业务候选范围规则的节点测速。页面显示“视频”，内部仍使用兼容组名 `HK-视频`；候选按 YouTube 探针的连续成功、中位延迟和抖动生成待生效建议。
