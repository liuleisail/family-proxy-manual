...
安装器还会启用 `family-platform-update-check.timer`：每天约 09:15 查询 RouterOS 当前官方通道、Z4Pro 自带极空间升级服务，并对 Mihomo、MosDNS 执行只读镜像检查。该任务只写入维护页状态；发现新版本才经现有 Telegram 告警推送一次，不会升级 RouterOS、ZOS、`apt` 软件包、Docker 镜像或重启设备。
