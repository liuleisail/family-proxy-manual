...
- DNS 概览版面优化：解析路径独占一行（全宽）；常用域名与活跃设备下移到竞速区下方；概览内容用纵向容器统一 18px 间距；上游地址单行（省略号）、竞速小字允许换行不截断；国内上游/国外上游竞速数据改为点击解析路径卡片内问号图标后以**居中弹窗**展示（原固定竞速卡片与卡片内联面板均已移除）。弹窗容器 `.dns-race-modal`：宽 `min(760px, calc(100% - 28px))`、最大高 `min(720px, calc(100vh - 40px))`、内容可滚动，列表沿用 `.dns-race-columns/.dns-race-row` 四列网格；移动端沿用现有 760px 断点折叠布局。弹窗遮罩 `.dns-race-backdrop` 限定在右侧内容区（`left: var(--sidebar-w)`，跟随 244/214/0px 断点），不遮挡左侧固定菜单栏。
