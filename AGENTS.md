...
- 机场候选池（切换状态页压缩）：当前出口下每个出口组从整张卡片（标题+4 个信息块）改为紧凑表格行（`.runtime-row` 五列：出口组/策略/实际节点/来源/最近探测，行高约 58px），信息不变但整体高度大幅缩短；移动端表头隐藏、每行两列堆叠。旧 `.runtime-grid/.runtime-card/.runtime-facts` 样式已移除。表格嵌在 `surface-card failsafe-card` 卡片内（与上方“故障回退”卡同款卡头/说明/列表结构），保持整页卡片风格一致。
